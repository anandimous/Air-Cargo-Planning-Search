...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
            ...
            for cr in self.cargos:
                for pl in self.planes:
                    for ap in self.airports:
                        precond_pos = [expr("At({}, {})".format(cr, ap)), expr("At({}, {})".format(pl, ap))]
                        precond_neg = []
                        effect_add = [expr("In({}, {})".format(cr, pl))]
                        effect_rem = [expr("At({}, {})".format(cr, ap))]
                        load = Action(expr("Load({}, {}, {})".format(cr, pl, ap)), [precond_pos, precond_neg], [effect_add, effect_rem])
                        loads.append(load)
            return loads
        ...
        def unload_actions():
            ...
            for cr in self.cargos:
                for pl in self.planes:
                    for ap in self.airports:
                        precond_pos = [expr("In({}, {})".format(cr, pl)), expr("At({}, {})".format(pl, ap))]
                        precond_neg = []
                        effect_add = [expr("At({}, {})".format(cr, ap))]
                        effect_rem = [expr("In({}, {})".format(cr, pl))]
                        unload = Action(expr("Unload({}, {}, {})".format(cr, pl, ap)), [precond_pos, precond_neg], [effect_add, effect_rem])
        ...
        for action in self.actions_list: ## For every available action
            ...
            for clause in action.precond_pos:   ## Check if positive preconditions are in the list of true literals in kb
                if clause not in kb.clauses:
                    is_possible = False
            for clause in action.precond_neg:   ## Check if the negative preconditions are in kb
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        executed.
        """
        # TODO TBD in Alternate Approach
        count = 0
        kb = PropKB()
